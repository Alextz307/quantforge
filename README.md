...
- **Empirical-study orchestrator.** `experiment study run` sweeps every strategy across its compatible universes, tunes each leg, evaluates an untouched holdout, and runs per-universe cross-strategy comparisons. It is resumable via `study_state.json` and isolates per-leg failures; `experiment study report` consolidates the legs into master / per-universe / holdout rankings, a dev-vs-holdout scatter, and importance figures.
