...
class TestVolatilityTargetingStrategy:
    ...
    def test_update_extends_training_window(
        ...
        combined window while preserving ``train_start`` and ``fit_timestamp``."""
        split = len(train_df) // 2
        initial = train_df.iloc[:split]
        new = train_df.iloc[split:]

        s = _build_strategy(synthetic_feature_columns)
        s.train(initial)
        first_meta = s.training_metadata
        assert first_meta is not None
        assert first_meta.n_train_samples == len(initial)

        s.update(new)
        second_meta = s.training_metadata
        assert second_meta is not None
        assert second_meta.n_train_samples == len(initial) + len(new)
