...
describe("Pagination", () => {
  it("shows the current range and total", () => {
    renderWithProviders(<Pagination total={TOTAL} limit={LIMIT} offset={0} onOffset={vi.fn()} />);
  ...
  it("disables Next on the last (partial) page", () => {
    renderWithProviders(
      <Pagination total={TOTAL} limit={LIMIT} offset={LAST_PAGE_OFFSET} onOffset={vi.fn()} />,
    );
  ...
  it("advances by one page on Next and back on Previous", async () => {
    ...
    renderWithProviders(
      <Pagination total={TOTAL} limit={LIMIT} offset={PAGE_TWO_OFFSET} onOffset={onOffset} />,
    );
  ...
  it("keeps Previous reachable on an empty out-of-range page", () => {
    ...
    renderWithProviders(
      <Pagination total={SHRUNK_TOTAL} limit={LIMIT} offset={PAGE_TWO_OFFSET} onOffset={vi.fn()} />,
    );
    expect(screen.getByRole("button", { name: /previous/i })).toBeEnabled();
    expect(screen.getByRole("button", { name: /next/i })).toBeDisabled();
  });
