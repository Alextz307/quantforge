...
from webapp.backend.tests.conftest import make_synthetic_study
...
def test_list_studies_sorts_newest_first(tmp_path: Path) -> None:
    ...
    summaries = list_studies(root)

    assert [s.name for s in summaries] == [NEWER_NAME, OLDER_NAME]


def test_list_studies_surfaces_completion(tmp_path: Path) -> None:
    root = tmp_path / "experiment_results"
...
def test_get_study_returns_full_detail(tmp_path: Path) -> None:
    root = tmp_path / "experiment_results"
    make_synthetic_study(
        root / "studies",
        name=NEWER_NAME,
    ...
    detail = get_study(root, NEWER_NAME)
...
def test_get_study_raises_for_unknown_name(tmp_path: Path) -> None:
    root = tmp_path / "experiment_results"
    make_synthetic_study(root / "studies", name=NEWER_NAME)

    with pytest.raises(StudyNotFoundError):
        get_study(root, "missing_study")
