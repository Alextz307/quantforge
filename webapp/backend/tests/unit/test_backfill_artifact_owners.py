...
def _seed_admin(conn: sqlite3.Connection) -> int:
    user = create_user(conn, username=_ADMIN_USERNAME, password=_ADMIN_PASSWORD, role=Role.ADMIN)
    return int(user.id)
...
def test_backfill_attributes_every_artifact(
    ...
    plans = backfill(db_conn, username=_ADMIN_USERNAME, store_root=webapp_store, dry_run=False)
    ...
    inserted = db_conn.execute("SELECT kind, status, user_id, experiment_id FROM jobs").fetchall()
...
def test_backfill_is_idempotent(db_conn: sqlite3.Connection, webapp_store: Path) -> None:
    _seed_admin(db_conn)
    first = backfill(db_conn, username=_ADMIN_USERNAME, store_root=webapp_store, dry_run=False)
    assert first
    first_count = db_conn.execute("SELECT COUNT(*) AS n FROM jobs").fetchone()["n"]

    second = backfill(db_conn, username=_ADMIN_USERNAME, store_root=webapp_store, dry_run=False)
    assert second == []
    second_count = db_conn.execute("SELECT COUNT(*) AS n FROM jobs").fetchone()["n"]
    assert second_count == first_count


def test_dry_run_writes_nothing(db_conn: sqlite3.Connection, webapp_store: Path) -> None:
    _seed_admin(db_conn)
    plans = backfill(db_conn, username=_ADMIN_USERNAME, store_root=webapp_store, dry_run=True)
    assert plans
    count = db_conn.execute("SELECT COUNT(*) AS n FROM jobs").fetchone()["n"]
    assert count == 0


def test_unknown_user_raises(db_conn: sqlite3.Connection, webapp_store: Path) -> None:
    with pytest.raises(click.ClickException, match="not found"):
        backfill(db_conn, username="ghost", store_root=webapp_store, dry_run=False)


def test_empty_store_root_is_no_op(db_conn: sqlite3.Connection, tmp_path: Path) -> None:
    _seed_admin(db_conn)
    empty_root = tmp_path / "empty_store"
    empty_root.mkdir()
    plans = backfill(db_conn, username=_ADMIN_USERNAME, store_root=empty_root, dry_run=False)
    assert plans == []
    count = db_conn.execute("SELECT COUNT(*) AS n FROM jobs").fetchone()["n"]
    assert count == 0


def test_existing_jobs_row_is_preserved(db_conn: sqlite3.Connection, webapp_store: Path) -> None:
    """
    An artifact that already has an owner is not overwritten.
    """

    admin_id = _seed_admin(db_conn)
    bob = create_user(db_conn, username="bob", password="bob-password", role=Role.USER)
    ...
    backfill(db_conn, username=_ADMIN_USERNAME, store_root=webapp_store, dry_run=False)
