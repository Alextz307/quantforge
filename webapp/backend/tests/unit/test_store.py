...
EXPECTED_FLAT_LABEL = "flat_store/runs"
...
def test_iter_run_dirs_finds_both_layouts(tmp_path: Path) -> None:
    ...
    make_synthetic_run(root / "flat_store" / "runs", experiment_id=FLAT_ID)
...
def test_store_label_for_flat_layout(tmp_path: Path) -> None:
    root = tmp_path / "experiment_results"
    run_dir = make_synthetic_run(root / "flat_store" / "runs", experiment_id=FLAT_ID)
