...
def test_list_truncates_page_to_limit(authed_client: TestClient, webapp_store: Path) -> None:
    # Seed a second study so the total exceeds a limit of one; the page must clip
    # to `limit` while `total` reports the full count. Catches a limit/offset arg
    # swapped into paginate() that the single-row fixture can't.
    make_synthetic_study(webapp_store / "studies", name="second")
