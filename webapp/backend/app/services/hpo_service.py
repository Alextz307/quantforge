...
def find_live_job_for(conn: sqlite3.Connection, wire_id: str) -> str | None:
    ...
    register per-leg TUNE rows, so a basename match against a nested
...
def _summary_from_trials(
    ...
    return HpoSummary(
        ...
        best_config_reserves_holdout=(has_best_config and best_config_reserves_holdout(study_dir)),
    )
