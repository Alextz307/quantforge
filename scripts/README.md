...
User-facing CLIs (experiment, deploy) and stdlib-only drift guards
that protect invariants between two on-disk sources of truth (CI vs.
pyproject, Python vs. C++ constants).
...
| `study report --study-dir <path>` | `<study_dir>/{tables,plots,manifest.json}` | Walk a completed study tree; emit master / per-universe / holdout rankings (`.tex`+`.csv`), strategyxuniverse heatmap, dev-vs-holdout scatter, and per-universe equity-overlay / per-leg holdout-equity copies. Read-only with respect to the per-leg tree. |
