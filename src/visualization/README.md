...
| `StudyReportReporter.generate_full_report(report, out_dir, *, publish_label=None)` | Cross-leg consolidated artefacts: master / per-universe / holdout rankings (`.tex` + `.csv`), strategy x universe heatmap, dev-vs-holdout scatter, per-universe equity-overlay copies, per-leg holdout-equity copies. |
