...
| `SharpeObjective()` | The sole HPO objective: reads `sharpe_mean` (mean-of-folds Sharpe) from the aggregate-metrics dict. MAXIMIZED. |
...
| `objectives.py` | `SharpeObjective` - the single objective adapter over the aggregate-metrics dict. |
...
config, the tuner refuses, since continuing under a different
config would invalidate the trial history.
