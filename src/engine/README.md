...
fold's test window. The invariant: `train_end < test_start` for every
tracked component, the strategy and every wrapped leaf reached via
`get_all_training_metadata`. A `LeakageError` names the offending
component (`StrategyClass.origin`).
...
direct calls into `evaluate_walk_forward` are for tests that inject
mocks per component.
